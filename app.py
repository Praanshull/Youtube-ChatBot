...
/* ============================================================ */
    ...
    /* Container for navigation links in header */
    ...
    /* Create navigation using pseudo-elements */
...
1
    ...
    /* Headers in tech cards and info boxes - PURPLE */
    .tech-card h3, 
    .tech-card h4,
    .info-box h3,
    .info-box h4 {
    ...
    /* ALL OTHER TEXT in tech cards and info boxes - DARK */
    .tech-card p,
    .tech-card li,
    .tech-card strong,
    .tech-card span,
    ...
    /* Lists in tech cards and info boxes */
    .tech-card ul,
    .tech-card ol,
    .info-box ul,
    .info-box ol {
...
class Config:
    """Centralized configuration"""
        # Get token from Streamlit secrets (deployed) or .env file (local)
    try:
    ...
    if not HF_TOKEN:
        st.error("⚠️ HuggingFace token not found! Please set HF_TOKEN in .env file or Streamlit secrets.")
        st.stop()
    CHUNK_SIZE = 1000
    CHUNK_OVERLAP = 200
    RETRIEVAL_K = 10
...
def show_chat_page():
        ...
        if st.button("🚀 Initialize Chatbot", use_container_width=True):
            if video_input:
                try:
                    ...
                    st.query_params.update({"sidebar": "collapsed"})
                    st.rerun()
                except Exception as e:
                    st.error(f"❌ Error: {e}")
            else:
    ...
    if not st.session_state.initialized:
        ...
        <div class="info-box" style="text-align: center; padding: 3rem;">
            <h3>👋 Welcome!</h3>
            <p>Enter a YouTube video URL in the sidebar to get started.</p>
            <p style="color: #666;">The chatbot will analyze the video transcript and answer your questions.</p>
        </div>
        """, unsafe_allow_html=True)
        ...
        with col2:
            ...
            # Use built-in Streamlit container with fixed height
            ...
            with chat_container:
                            ...
                            if "queries" in message and len(message["queries"]) > 1:
                                with st.expander("🔍 View Generated Query Variations"):
                                    for idx, q in enumerate(message["queries"], 1):
                                        # Clean up query text - remove leading numbers
                                        clean_query = q.strip()
                                        import re
                                        clean_query = re.sub(r'^\d+\.\s*', '', clean_query)
                                        st.markdown(f"**Query {idx}:** {clean_query}")
                
                # If last message is from user and we need to generate response
                if st.session_state.messages and st.session_state.messages[-1]["role"] == "user":
                    # Show clean thinking indicator INSIDE chat container
                    with st.chat_message("assistant"):
                        st.markdown("🤔 **Analyzing video transcript...**")
                        
                        # Generate response
                        try:
                            last_question = st.session_state.messages[-1]["content"]
                            result = st.session_state.chain.invoke(last_question)
                            answer = result["answer"].strip()
                            queries = result["queries"]
                            
                            # Update conversation history
                            st.session_state.conversation_history += f"Q: {last_question}\nA: {answer}\n\n"
                ...
                if user_question:
                    ...
                    st.rerun()  # Rerun to show user message and trigger response generation
        ...
        #         # If last message is from user and we need to generate response
        #         if st.session_state.messages and st.session_state.messages[-1]["role"] == "user":
        #             # Show thinking indicator INSIDE chat container
        #             with st.chat_message("assistant"):
        #                 with st.spinner(""):
        #                     st.markdown("""
        #                         <div style="padding: 0.8rem; background: rgba(126, 34, 206, 0.1); 
        #                                     border-radius: 10px; border-left: 4px solid #7e22ce;">
        #                             <p style="margin: 0; color: #7e22ce; font-weight: bold;">
...
def add_top_navigation():
    """Add navigation links to top header"""
    current_page = st.session_state.current_page
    
    # Create navigation HTML
    nav_html = """
    <div class="top-nav">
        <a href="?page=chat" class="{}" onclick="return false;" id="nav-chat">
            💬 Chat
        </a>
        <a href="?page=tech" class="{}" onclick="return false;" id="nav-tech">
            🛠️ Tech Stack
        </a>
        <a href="?page=about" class="{}" onclick="return false;" id="nav-about">
            ℹ️ About
        </a>
    </div>
    ...
    <script>
        ...
        navLinks.forEach(link => {{
            link.addEventListener('click', function(e) {{
                ...
                // Update Streamlit query params
                const params = new URLSearchParams(window.location.search);
                params.set('nav_click', page);
    ...
    """.format(
        'active' if current_page == "💬 Chat" else '',
        'active' if current_page == "🛠️ Tech Stack" else '',
        'active' if current_page == "ℹ️ About" else ''
    )
...
def main():
    ...
    # Check for navigation clicks from header
    query_params = st.query_params
    if "nav_click" in query_params:
        nav_page = query_params["nav_click"]
        
        # Map navigation IDs to page names
        page_map = {
            "chat": "💬 Chat",
            "tech": "🛠️ Tech Stack",
            "about": "ℹ️ About"
        ...
        if nav_page in page_map:
            st.session_state.current_page = page_map[nav_page]
    ...
    page = st.sidebar.radio(
        ...
        index=["💬 Chat", "🛠️ Tech Stack", "ℹ️ About"].index(st.session_state.current_page) if st.session_state.current_page in ["💬 Chat", "🛠️ Tech Stack", "ℹ️ About"] else 0,
